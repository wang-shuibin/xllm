...
void ResidualCache::init(const DiTCacheConfig& cfg) {
  CHECK_GE(cfg.residual_cache.skip_interval_steps, 0)
      << "skip_interval_steps must be >= 0";
  CHECK_GE(cfg.residual_cache.dit_cache_start_steps, 0)
      << "dit_cache_start_steps must be >= 0";
  CHECK_GE(cfg.residual_cache.dit_cache_end_steps, 0)
      << "dit_cache_end_steps must be >= 0";
  CHECK_GE(cfg.residual_cache.dit_cache_start_blocks, 0)
      << "dit_cache_start_blocks must be >= 0";
  CHECK_GE(cfg.residual_cache.dit_cache_end_blocks, 0)
...
bool ResidualCache::on_before_step(const CacheStepIn& stepin) {
  ...
  if (!cache_validation() || current_step_ < dit_cache_start_steps_ ||
  ...
  use_cache_ =
      ((current_step_ - dit_cache_start_steps_) % skip_interval_steps_ != 0);
